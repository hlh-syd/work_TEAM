...
def add_fixed_24m_endpoint(
    ...
    df["death_by_24m_observed"] = observed_status.astype(bool)
    df["death_by_24m"] = np.where(observed_status, event_by_tau, np.nan)
    df["early_censored_before_24m"] = (~observed_status).astype(bool)
    df["survived_observed_24m"] = (df["time_months"] > tau).astype(bool)
    return df
...
def compute_ipcw_weights(endpoint: pd.DataFrame, tau: float) -> pd.DataFrame:
    ...
    label_observed = df["death_by_24m_observed"].to_numpy(dtype=bool)
...
def compute_pseudo_observations(endpoint: pd.DataFrame, tau: float, max_n_exact: int = 1200) -> pd.DataFrame:
    ...
    df["pseudo_risk_24m"] = pseudo
    df["pseudo_risk_24m_clipped"] = np.clip(pseudo, 0.0, 1.0)
    return df
...
def censor_safe_binary_training(endpoint: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    ...
    y = endpoint.loc[mask, "death_by_24m"].astype(int)
    w = endpoint.loc[mask, "ipcw_weight_24m"].astype(float)
    return y, w
...
def train_only_cv_metrics_for_augmented_logistic(
    ...
    if y_real.nunique() < 2 or len(y_real) < 40:
        return {"auc_24m_observed": float("nan"), "average_precision_24m": float("nan"), "brier_24m_ipcw": float("nan")}
    ...
    for fold_id, (train_pos, val_pos) in enumerate(splitter.split(X_real_obs, y_real), start=1):
        ...
        y_val = endpoint_real.loc[fold_val_ids, "death_by_24m"].astype(int).to_numpy()
        w_val = endpoint_real.loc[fold_val_ids, "ipcw_weight_24m"].replace(0, np.nan).fillna(1.0).to_numpy(float)
        ...
        try:
            ...
            pass
    return {
        "auc_24m_observed": float(np.nanmean(auc_scores)) if auc_scores else float("nan"),
        "average_precision_24m": float(np.nanmean(ap_scores)) if ap_scores else float("nan"),
        "brier_24m_ipcw": float(np.nanmean(brier_scores)) if brier_scores else float("nan"),
    }
...
def train_only_cv_auc_for_augmented_logistic(
    ...
    return float(metrics.get("auc_24m_observed", float("nan")))
...
def evaluate_24m_predictions(
    ...
    result: Dict[str, Any] = {
        ...
        "early_censored_before_24m": int(eval_endpoint["early_censored_before_24m"].sum()),
    }
    valid_risk = np.isfinite(risk)
    observed = eval_endpoint["death_by_24m_observed"].to_numpy(bool) & valid_risk
    if observed.sum() >= 5 and pd.Series(eval_endpoint.loc[observed, "death_by_24m"]).nunique() == 2:
        y = eval_endpoint.loc[observed, "death_by_24m"].astype(int).to_numpy()
        scores = risk[observed]
        weights = eval_endpoint.loc[observed, "ipcw_weight_24m"].replace(0, np.nan).fillna(1.0).to_numpy(float)
        try:
            result["auc_24m_observed"] = float(roc_auc_score(y, scores, sample_weight=weights))
        except Exception:
            result["auc_24m_observed"] = float("nan")
        try:
            result["brier_24m_ipcw"] = float(np.average((y - scores) ** 2, weights=weights))
        except Exception:
            result["brier_24m_ipcw"] = float("nan")
        try:
            result["average_precision_24m"] = float(average_precision_score(y, scores, sample_weight=weights))
        except Exception:
            result["average_precision_24m"] = float("nan")
        ...
        result.update(
            {
                "auc_24m_observed": float("nan"),
                "brier_24m_ipcw": float("nan"),
                "average_precision_24m": float("nan"),
            }
        )
    ...
    if (
        ...
        try:
            ...
            result["time_dependent_auc_24m"] = float(auc_t[0])
            result["mean_auc_24m"] = float(mean_auc)
        except Exception:
            result["time_dependent_auc_24m"] = float("nan")
            result["mean_auc_24m"] = float("nan")
    else:
        result["uno_cindex_ipcw"] = float("nan")
        result["time_dependent_auc_24m"] = float("nan")
        result["mean_auc_24m"] = float("nan")
...
def choose_training_threshold(endpoint_train: pd.DataFrame, risk_train: np.ndarray) -> Optional[float]:
    valid = endpoint_train["death_by_24m_observed"].to_numpy(bool) & np.isfinite(risk_train)
    if valid.sum() < 10:
        return None
    y = endpoint_train.loc[valid, "death_by_24m"].astype(int).to_numpy()
...
def observed_binary_for_plot(endpoint: pd.DataFrame, risk: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    valid = endpoint["death_by_24m_observed"].to_numpy(bool) & np.isfinite(risk)
    if valid.sum() == 0:
        return np.asarray([], dtype=int), np.asarray([], dtype=float), np.asarray([], dtype=float)
    y = endpoint.loc[valid, "death_by_24m"].astype(int).to_numpy()
    scores = np.asarray(risk[valid], dtype=float)
    weights = endpoint.loc[valid, "ipcw_weight_24m"].replace(0, np.nan).fillna(1.0).to_numpy(float)
...
def run_causal_screening(
    ...
    outcome = endpoint_train.reindex(common)["pseudo_risk_24m_clipped"].astype(float)
    rows: List[Dict[str, Any]] = []
...
class SmoteLikeAugmenter:
    ...
    def fit(self, X: pd.DataFrame, condition: Any) -> "SmoteLikeAugmenter":
        ...
        if isinstance(condition, pd.DataFrame):
            labels = condition.reindex(data.index)["death_by_24m"].round().clip(0, 1).astype(int).to_numpy()
        else:
...
def build_survival_gan_conditions(endpoint: pd.DataFrame, index: Sequence[Any]) -> pd.DataFrame:
    ...
    death = ep.get("death_by_24m", pd.Series(0.0, index=ep.index)).astype(float)
    event = ep.get("event", death).astype(float)
    time_months = ep.get("time_months", pd.Series(TAU_MONTHS, index=ep.index)).astype(float)
    pseudo = ep.get("pseudo_risk_24m_clipped", death).astype(float)
    weight = ep.get("ipcw_weight_24m", pd.Series(1.0, index=ep.index)).astype(float)
    cond = pd.DataFrame(index=ep.index)
    cond["death_by_24m"] = death.fillna(0.0).clip(0.0, 1.0)
    cond["event"] = event.fillna(cond["death_by_24m"]).clip(0.0, 1.0)
    ...
    cond["pseudo_risk_24m_clipped"] = pseudo.replace([np.inf, -np.inf], np.nan).fillna(cond["death_by_24m"]).clip(0.0, 1.0)
    cond["ipcw_weight_24m"] = weight.replace([np.inf, -np.inf], np.nan).fillna(1.0).clip(lower=EPS)
...
def _sample_condition_rows(
    ...
    if sampling_strategy == "event_only":
        ...
        cond["death_by_24m"] = 1.0
        cond["event"] = 1.0
        cond["pseudo_risk_24m_clipped"] = cond["pseudo_risk_24m_clipped"].clip(lower=0.5)
    ...
    if sampling_strategy == "risk_stratified" and "pseudo_risk_24m_clipped" in endpoint_train:
        pseudo = endpoint_train.loc[train_idx, "pseudo_risk_24m_clipped"].astype(float).replace([np.inf, -np.inf], np.nan)
        valid = pseudo.dropna()
        if len(valid) >= 3:
            try:
...
class FeatureSpaceGAN:
    ...
    def fit(self, X: pd.DataFrame, condition: Any, endpoint: Optional[pd.DataFrame] = None) -> "FeatureSpaceGAN":
        ...
        if endpoint is not None:
            y_ae = endpoint.reindex(data.index)["death_by_24m"].fillna(0).astype(float).to_numpy()
            w_ae = endpoint.reindex(data.index)["ipcw_weight_24m"].replace(0, np.nan).fillna(1.0).astype(float).to_numpy()
        else:
...
def train_only_gan_augmentation(
    ...
    if len(eligible_ids) < 40 or y_obs.nunique() < 2:
        audit.add("gan_augmentation", "SKIP", "Insufficient observed 24-month labels in TCGA_train_real.")
        ...
        try:
            ...
            for attempt in range(3):
                ...
                if isinstance(gan_model, (FeatureSpaceGAN, SurvivalConditionalTabularGAN, SmoteLikeAugmenter)):
                    ...
                    sample_condition = sampled_cond_df["death_by_24m"].to_numpy(int)
                X_syn_candidate = gan_model.sample(n_syn_candidate, sample_condition)
                if X_syn_candidate.empty:
                    continue


                syn_event_mask = sampled_cond_df["death_by_24m"].to_numpy(int)[: len(X_syn_candidate)] == 1
    ...
    endpoint_syn["death_by_24m"] = sampled_conditions["death_by_24m"].round().clip(0, 1).astype(float).to_numpy()
    endpoint_syn["death_by_24m_observed"] = False
    endpoint_syn["early_censored_before_24m"] = False
    endpoint_syn["ipcw_label_available"] = True
    endpoint_syn["ipcw_weight_24m"] = sampled_conditions["ipcw_weight_24m"].replace(0, np.nan).fillna(1.0).astype(float).to_numpy()
    endpoint_syn["pseudo_risk_24m"] = sampled_conditions["pseudo_risk_24m_clipped"].clip(0.02, 0.98).astype(float).to_numpy()
    endpoint_syn["pseudo_risk_24m_clipped"] = endpoint_syn["pseudo_risk_24m"].clip(0.02, 0.98)
...
def select_train_only_gan_augmentation(
    ...
    for sampling_strategy in sampling_strategies:
        for ratio in candidate_ratios:
            ...
            cv_metrics = {"auc_24m_observed": float("nan"), "average_precision_24m": float("nan"), "brier_24m_ipcw": float("nan")}
            ...
            auc_gain = float(cv_metrics.get("auc_24m_observed", float("nan"))) - float(baseline_metrics.get("auc_24m_observed", float("nan")))
            ap_delta = float(cv_metrics.get("average_precision_24m", float("nan"))) - float(baseline_metrics.get("average_precision_24m", float("nan")))
            brier_delta = float(cv_metrics.get("brier_24m_ipcw", float("nan"))) - float(baseline_metrics.get("brier_24m_ipcw", float("nan")))
            ...
            candidate = {
                ...
                "cv_auc_24m_observed": cv_metrics.get("auc_24m_observed", float("nan")),
                "cv_average_precision_24m": cv_metrics.get("average_precision_24m", float("nan")),
                "cv_brier_24m_ipcw": cv_metrics.get("brier_24m_ipcw", float("nan")),
            ...
            audit.add(
                ...
                f"train-only CV AUC={candidate['cv_auc_24m_observed']:.4f}, "
                f"AP={candidate['cv_average_precision_24m']:.4f}, Brier={candidate['cv_brier_24m_ipcw']:.4f}.",
                sampling_strategy=sampling_strategy,
                ratio=ratio,
    ...
    def _selection_key(candidate: Dict[str, Any]) -> Tuple[float, float, float, float, int]:
        ...
        brier = float(candidate.get("cv_brier_24m_ipcw", float("nan")))
    ...
    selected_report.update(
        {
            ...
            "train_only_cv_auc_24m_observed": selected["cv_auc_24m_observed"],
            "train_only_cv_average_precision_24m": selected["cv_average_precision_24m"],
            "train_only_cv_brier_24m_ipcw": selected["cv_brier_24m_ipcw"],
            "candidate_ratios": list(candidate_ratios),
            "sampling_strategies": list(sampling_strategies),
            "candidates": [
                {k: v for k, v in c.items() if k not in {"X_aug", "endpoint_aug"}}
    ...
    audit.add(
        ...
        f"train-only CV AUC={selected['cv_auc_24m_observed']:.4f}, "
        f"AP={selected['cv_average_precision_24m']:.4f}, Brier={selected['cv_brier_24m_ipcw']:.4f}.",
        selected_sampling_strategy=selected["sampling_strategy"],
        selected_aug_ratio=selected["ratio"],
        selected_method=selected.get("method", ""),
...
def load_external_cohorts(preprocessed_root: Path, tau: float, audit: AuditLog) -> Dict[str, pd.DataFrame]:
    ...
    for path in sorted(preprocessed_root.glob("*_os_clinical_endpoint_qc.tsv")):
        ...
        try:
            ...
            if "PATIENT_ID" not in df.columns:
                continue
            endpoint = add_fixed_24m_endpoint(df, tau=tau).set_index("PATIENT_ID", drop=False)
...
def write_environment_file(out_dir: Path) -> Path:
    ...
    content = """# Environment for 2YearsOS.py
python==3.11.x
numpy==1.26.4
pandas==2.2.2
scipy==1.13.1
...
def run_pipeline(cfg: PipelineConfig) -> Dict[str, Any]:
    ...
    audit = AuditLog()
    run_dir = ensure_dir(cfg.output_root / f"2YearsOS_{now_stamp()}")
    ...
    tcga = add_fixed_24m_endpoint(read_tsv(tcga_clin_path), tau=cfg.tau_months).set_index("PATIENT_ID", drop=False)
    tcga = compute_ipcw_weights(tcga, cfg.tau_months)
    tcga = compute_pseudo_observations(tcga, cfg.tau_months)
    tcga.to_csv(tables_dir / "tcga_24m_endpoint_ipcw_pseudo.tsv", sep="\t", index=False)
    audit.add(
        "endpoint",
        "OK",
        "Constructed 24-month endpoint; early censored cases retain missing fixed-time label.",
        n=int(len(tcga)),
        early_censored=int(tcga["early_censored_before_24m"].sum()),
    )
    ...
    if split_path.exists():
        ...
        observed_label = tcga["death_by_24m_observed"].astype(int).astype(str) + "|" + tcga["event"].astype(str)
    ...
    for feature_set_name, cols in model_feature_sets.items():
        ...
        for model_name, (model_kind, model) in models_to_fit.items():
            ...
            train_metrics = evaluate_24m_predictions(train_endpoint, train_endpoint, risk_train, cfg.tau_months, threshold)
            val_metrics = evaluate_24m_predictions(train_endpoint, val_endpoint, risk_val, cfg.tau_months, threshold)
            model_rows.append(
                {
                    "model_name": model_name,
    ...
    sort_cols = ["internal_validation_uno_cindex_ipcw", "internal_validation_time_dependent_auc_24m", "internal_validation_brier_24m_ipcw"]
    ...
    for cohort_name, ext_endpoint in external_cohorts.items():
        ...
        metrics = evaluate_24m_predictions(train_endpoint, ext_endpoint, risk, cfg.tau_months, selected_bundle["threshold"])
        ...
        for candidate_name, candidate_bundle in fitted_models.items():
            ...
            candidate_metrics = evaluate_24m_predictions(
                train_endpoint,
                ext_endpoint,
                candidate_risk,
                cfg.tau_months,
    ...
    external_generalization = {
        ...
        "selected_model_external_mean_auc_24m": (
            float(external_metrics["time_dependent_auc_24m"].dropna().mean())
            if not external_metrics.empty and "time_dependent_auc_24m" in external_metrics
            else None
        ),
        "publishability_note": (
    ...
    leakage = {
        ...
        "rules": [
            ...
            "Early censoring before 24 months is not labelled as 2-year survivor.",
        ],
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="CRC 2-year OS risk prediction pipeline.")
