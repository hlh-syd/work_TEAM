...
def lifelines_survival_estimator(model: Any, x: Any) -> Any:
    """Return a callable that evaluates the lifelines Cox survival function S(t|x) at given horizons."""
    try:
        surv_funcs = model.predict_survival_function(x)  # DataFrame: index=time, columns=samples
        times_idx = surv_funcs.index.values.astype(float)
        values = surv_funcs.values.T  # (n_samples, n_times)
        def _estimate(horizons: np.ndarray) -> np.ndarray:
            out = np.empty((values.shape[0], len(horizons)), dtype=float)
            for j, t in enumerate(horizons):
                idx = int(np.searchsorted(times_idx, t, side="right")) - 1
                if idx < 0:
                    out[:, j] = 1.0
                else:
                    out[:, j] = values[:, idx]
...
def sksurv_survival_estimator(model: Any, x: Any) -> Any:
    """Return a callable that evaluates the sksurv survival function S(t|x) at given horizons."""
    try:
        surv_fns = model.predict_survival_function(x)  # list of step functions
        def _estimate(horizons: np.ndarray) -> np.ndarray:
            rows = []
            for sf in surv_fns:
                row = np.empty(len(horizons), dtype=float)
                for j, t in enumerate(horizons):
                    row[j] = float(sf(t))
                rows.append(row)
...
def run_multi_timepoint_analysis() -> Dict[str, Any]:
    ...
    for tau in TIMEPOINTS:
        ...
        if tau is not None:
            ...
            ep = clinical.copy()
            # Normalise column names so downstream code can use 'event' / 'time_months'
            if "OS_EVENT" in ep.columns and "event" not in ep.columns:
                ep["event"] = ep["OS_EVENT"].astype(int)
            if "OS_TIME_MONTHS" in ep.columns and "time_months" not in ep.columns:
                ep["time_months"] = pd.to_numeric(ep["OS_TIME_MONTHS"], errors="coerce")
            ep["split"] = ep["PATIENT_ID"].map(split_map)
            train_mask = ep["split"] == "train"
            val_mask = ep["split"] == "internal_validation"
            if train_mask.sum() == 0:
...
def run_full_pipeline():
    ...
    steps = [
        ("01 - Multi-omics Preprocessing & QC (+ IPCW)", lambda: main_step01()),
    ...
    print(f"  Successful: {successful}/8 steps")
    print()
    ...
    return 0 if successful == 8 else 1
